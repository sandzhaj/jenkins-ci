...
freeStyleJob("lxd-manager/2_create_app_container_dsl") {
  ...
  parameters {
    ...
    choiceParam('TEMPLATE_IMAGE', ['QA-Baseline-CentOS7-v03', 'QA-Baseline-CentOS7-v02', 'QA-Baseline-CentOS7-v04', 'none'], 'Select the base container image')
    choiceParam('PUPPET_BRANCH', ['qa', 'staging', 'production'], 'Select puppet branch')
    choiceParam('PROFILE_NAME', ['Corevist_QA_Containers', 'Corevist_QA_Magento', 'Corevist_QA_Firewall'], 'Select the container profile')
  ...
  steps {
      shell('''SETTINGS_FILE=\'app_container.cfg\'
...
echo "PUPPET_BRANCH=\"$PUPPET_BRANCH\"" >> $SETTINGS_FILE
echo "PROFILE_NAME=\"$PROFILE_NAME\"" >> $SETTINGS_FILE''')
  }
  ...
  publishers {
    publishOverSsh {
      server('lxd-cluster') {
        transferSet {
            sourceFiles('app_container.cfg')
            remoteDirectory('tmp/')
            execCommand('''cat ~/tmp/app_container.cfg
create_app_container.sh ~/tmp/app_container.cfg''')
            patternSeparator('[, ]+')
            execTimeout(0)
            execInPty(true) 
        }
