...
clients.each { Map config ->
  ...
  pipelineJob("${config.client_name}/11-prod-magento-deploy") {
    ...
    properties {
      ...
      pipelineTriggers {
        triggers {
          upstream {
            upstreamProjects("${config.client_name}/magento_github_tagger")
          }
        }
      }
    }

    if ("target_magento_prod_env" in config) {
      target_env = config.target_magento_prod_env
    } else {
      target_env = ['prod-catalog']
    }
    ...
    definition {
      cpsScm {
        ...
        scriptPath('jobs/pipeline/11-prod-magento-deploy')
      }
    }
  }
}
