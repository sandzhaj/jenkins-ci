...
clients.each { Map config ->
  ...
  pipelineJob("${config.client_name}/magento_jira_after_deploy") {
    ...
    properties {
      ...
      pipelineTriggers {
        triggers {
          upstream {
            upstreamProjects("${config.client_name}/11-prod-magento-deploy")
          }
        }
      }
    }
    ...
    definition {
      cpsScm {
        ...
        scriptPath('jobs/pipeline/jira_after_deploy_trigger-pipeline')
      }
    }
  }
}
